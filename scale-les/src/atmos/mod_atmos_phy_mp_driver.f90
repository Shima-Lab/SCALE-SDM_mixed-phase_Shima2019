...
!! @li      2019-01-11 (S.Shima)      [add] flag to switch on/off use_tendency_mp
!<
!-------------------------------------------------------------------------------
...
!OCL XFILL
       ...
       if(use_tendency_mp .eqv. .false.)then
...
!OCL XFILL
          ...
          do i  = IS, IE
          do k  = KS, KE
             DENS_t_MP(k,i,j) = 0.0_RP
             MOMZ_t_MP(k,i,j) = 0.0_RP
             MOMX_t_MP(k,i,j) = 0.0_RP
             MOMY_t_MP(k,i,j) = 0.0_RP
             RHOT_t_MP(k,i,j) = 0.0_RP
          enddo
          enddo
          enddo
